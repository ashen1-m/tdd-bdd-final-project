...
describe('Product Routes', () => {
  ...
  describe('LIST Products', () => {
    ...
    test('should filter products by name', async () => {
      ...
      const response = await request(app)
        .get(BASE_URL)
        .query({ name: 'My Special Laptop' }) // Fixed mismatch in name search
        .expect(200);
    ...
    test('should filter products by category', async () => {
      ...
      const response = await request(app)
        .get(BASE_URL)
        .query({ category: 'TOOLS' }) // Utilizing supertest's .query() method for clean encoding
        .expect(200);
    ...
    test('should filter products by availability', async () => {
      ...
      const response = await request(app)
        .get(BASE_URL)
        .query({ available: true }) // Fixed query param from availability to available
        .expect(200);

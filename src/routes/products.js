...
// -------------------------------------------------------------------------
// LIST PRODUCTS (Consolidated endpoint handling ALL filters dynamically)
// -------------------------------------------------------------------------
router.get('/', async (req, res) => {
  try {
    // Checking for both available and availability based on your test iterations
    const { category, availability, available, name } = req.query;
    const where = {};
    ...
    // Support both `?available=true` and `?availability=true`
    const isAvailable = available !== undefined ? available : availability;
    if (isAvailable !== undefined) {
      where.available = isAvailable === 'true';
    }
